...
describe('taskAcceptedHandler',()=>{
    it('taskAcceptedHandler is called with client call true',()=>{
        waitForConferenceParticipants.mockReturnValue(Promise.resolve(['worker',{participantType : 'customer', callSid : '123'},'123','12334']))
        let spy = jest.spyOn(RecordingService, 'startDualChannelRecording').mockReturnValue(mockRecording);
    ...
    it('taskAcceptedHandler is called with client call true and no callSid value',()=>{
        waitForConferenceParticipants.mockReturnValue(Promise.resolve(['worker',{participantType : 'customer'},'123','12334']))
        let spy = jest.spyOn(RecordingService, 'startDualChannelRecording').mockReturnValue(mockRecording);
        let flex: typeof Flex = Flex;
        let manager: Flex.Manager = Flex.Manager.getInstance();
        const mockTask= {
