...
//import { pauseRecording } from 'pause-recording/helpers/pauseRecordingHelper';
...
const PauseStatusPanel = (props: OwnProps) => {
  ...
  const { pausedRecordings } = useSelector((state: AppState) => state[reduxNamespace]);
    ...
    <>
      {paused && (
        <Text as="p" textAlign="center" fontWeight="fontWeightBold" padding="space50">
          {PauseRecordingStrings[StringTemplates.RECORDING_PAUSED_LABEL]}
        </Text>
      )}
    </>
  );
